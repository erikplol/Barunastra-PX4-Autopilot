...
/dev/ttyUSB0 or 0.0.0.0:14550. Auto-detect serial if not given.')
    ...
    try:
        ...
        # Wait for response or a timeout (5 seconds)
        response_timeout = timer() + 1  # 5 seconds timeout
        while timer() < response_timeout:
            data = mav_serialport.read(4096)
            if data:
                sys.stdout.write(data)
                sys.stdout.flush()
                if "response_or_acknowledgment" in data:  # Replace with expected response substring
                    print("\nCommand executed successfully.")
                    break
        else:
            print("\nNo response or acknowledgment received within the timeout period.")
